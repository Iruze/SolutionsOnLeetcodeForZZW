...
def helper(root):
    ...
    left = helper(root.left)        # 1. 后续遍历-递归求出左、右分支结果
    right = helper(root.right)
    ...
    <>						# 具体的逻辑
    ...
    self.ans = max(self.ans, left + right)      # 2. 当前节点和左右节点拼接，其结果参与全局的结果比较
    return max(left, right)    	  		# 3. 当前节点作为其他节点的分支，求出作为节点的结果（比较拗口哈）
```

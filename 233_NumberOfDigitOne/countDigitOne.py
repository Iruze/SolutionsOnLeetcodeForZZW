...
class Solution:
    def countDigitOne(self, n: int) -> int:
        if n <= 0:
            return 0
        strnum = str(n)
        highnum = int(strnum[0])
        size = len(strnum)
        withouthigh = n - highnum * pow(10, size - 1)
        if highnum > 1:
            first_cnt = pow(10, size - 1)
        elif highnum == 1:
            first_cnt = withouthigh + 1
        other_cnt = highnum * (size - 1) * pow(10, size - 2)
        print(first_cnt, other_cnt, self.countDigitOne(withouthigh))
        return int(first_cnt + other_cnt + self.countDigitOne(withouthigh))
